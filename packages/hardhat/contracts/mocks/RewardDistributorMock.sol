...
contract RewardDistributorMock is UUPSUpgradeable, OwnableUpgradeable {
    ...
    event Recovered(uint indexed recoverAmount);
     ...
     */
    function recoverFees(uint _recoverAmount) external onlyOwner {
        ...
        rewardToken.safeTransfer(owner(), _recoverAmount);
        emit Recovered(_recoverAmount);
    }
